...
http.createServer(function (req, res) {
  ...
  if (pathname == '/') {
    ...
    //console.log( now + ':' + 'path name /' + pathname);
    return;
    ...
    if (stats && stats.isFile()) {
      ...
      //console.log( now + ':' + 'path name js' + pathname);
      return;
    }
  }
    ...
    res.end();
    //console.log( now + ':' + 'path name color - ' + pathname);
    return;
  }
  res.writeHead(404, {'Content-Type': 'text/plain'});
  res.write('404 Not Found\n');
  console.log( now + ':' + 'path name error' + pathname);
  res.end();
//}).listen(8765, 'localhost'); // Not restricting to localhost
}).listen(8765, '0.0.0.0');

console.log( now + ':' + 'mosaic server running on port 8765');
